...
namespace QueryProcessor.Operations
{
    internal class SelectSystemColumns
    {
        public OperationResult Execute()
        {
                ...
                foreach (var table in tables)
                {
                    ...
                    foreach (var column in columns)
                    {
                        var row = new Dictionary<string, string>
                        {
                            ...
                            { "Size", column.MaxSize.HasValue ? column.MaxSize.Value.ToString() : "N/A" }
                        };
                        tableData.Add(row);
                    }
                }
            ...
            {
                return new OperationResult { Status = OperationStatus.Success, Message = "No se encontraron columnas en las bases de datos." };
            }
        ...
        private string BuildTable(List<Dictionary<string, string>> tableData)
        {
            ...
            var columns = new List<string> { "Database", "Table", "Column", "Type", "Size" };
